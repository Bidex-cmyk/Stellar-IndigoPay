...
* **backend/security:** harden `merkleTree.js` against Merkle-tree second-preimage/odd-node forgeries — add leaf/node domain-separation prefixes, duplicate-and-hash trailing odd nodes instead of promoting them, and commit the leaf count into the published root so proofs bind the tree shape (closes #675)
