...
* **backend:** re-anchor the admin audit hash chain when retention prunes old rows, so `verifyChain` keeps validating the surviving suffix after the genesis row is purged (closes #685)
