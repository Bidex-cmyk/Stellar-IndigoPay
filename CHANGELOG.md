...
- **contracts/escrow-contract:** unify the two divergent dispute models — the deprecated `dispute_job` now mirrors its dispute onto every unreleased milestone and `dispute_milestone` keeps `job.disputed` in sync, so interleaving either dispute entrypoint with either resolution path (`resolve_dispute` / `resolve_milestone_dispute`) no longer strands a job (closes #613)
